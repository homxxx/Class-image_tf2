...
def write_csv(metrics_out_path, test_annotation_path, root_path, ii, mask):
    ...
    csv_writer.writerow(tabel_head_list)
    ...
    for index, line in tqdm(enumerate(lines), total=total):
        ...
        try:
            ...
            csv_writer.writerow(tabel_write_list)
        except:
...
def eva_roc(metrics_out_path):
    ...
    # print(true_y)
    # true_y = to_categorical(true_y, num_classes = 23)
    ...
    for i in range(n_classes):
        ...
        try:
            ...
            print('error confidence_interval')
    ...
    if math.isnan(roc_auc["macro"]) == False:
        plt.plot(fpr["macro"], tpr["macro"], label='macro-average ROC curve (area = {0:0.4f})'
    ...
    for i, color in zip(range(n_classes), colors):
        # print('test-> ', roc_auc[i], ' - ', math.isnan(roc_auc[i]))
        if math.isnan(roc_auc[i]) == False:
            plt.plot(fpr[i], tpr[i], color=color, lw=lw, label=labels[i] + '(area = {0:0.4f})'.format(roc_auc[i]))
...
if __name__ == '__main__':
    ...
    print(class_names)
    # Data path
    test_annotation_path = 'data_path_txt/data_path_test1.4.txt'

    root_path = 'metrics_out/result1'
    if not os.path.exists(root_path):
        os.makedirs(root_path)

    metrics_out_path = root_path
    if not os.path.exists(metrics_out_path):
        os.makedirs(metrics_out_path)

    write_csv(metrics_out_path, test_annotation_path, root_path, ii=0, mask=None)
    eva_roc(metrics_out_path)
